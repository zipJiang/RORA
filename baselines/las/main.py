...
def load_data(args, data_name, tokenizer):
    ...
    if data_name == "COSE":
        if not os.path.exists(os.path.join(args.data_dir, 'train.csv')):
            os.makedirs(args.data_dir, exist_ok=True)
            QA_data_utils.convert_cose_dataset_to_csv(data_name, data_dir='data/processed_datasets/cose', target_data_dir=args.data_dir, split='train', target_split='train')
            QA_data_utils.convert_cose_dataset_to_csv(data_name, data_dir='data/processed_datasets/cose', target_data_dir=args.data_dir, split='validation', target_split='dev')
            QA_data_utils.convert_cose_dataset_to_csv(data_name, data_dir='data/processed_datasets/cose', target_data_dir=args.data_dir, split='validation', target_split='test')
        read_function = QA_data_utils.read_ecqa
        if 't5' in args.task_pretrained_name:
            prep_function = QA_data_utils.get_tensors_for_T5_split
        elif 'bert' in args.task_pretrained_name:
    ...
    test_examples = read_function(args,
                            ...
                            explanations_to_use = args.explanations_to_use, 
                            # labels_to_use = None if ('QA' in data_name and args.labels_to_use == 'label') else args.labels_to_use,
                            labels_to_use = args.labels_to_use,
                            version = version,
                            split='test')
...
def train_or_eval_epoch(args, device, dataloader, stats_dict, multi_gpu, 
    ...
    for step, batch in enumerate(dataloader):
        ...
        if 'strategyqa' in args.data_dir.lower():
            ...
            num_choices = 5
        elif 'cose' in args.data_dir.lower():
            num_choices = 5
        else:
            num_choices = 3
...
if __name__ == "__main__":
    ...
    # hyperparams
    parser.add_argument("--train_batch_size", default=64, type=int, help="Total batch size for training. Effective batch size is this times grad_accumulation_factor")
    ...
    parser.add_argument("--dev_batch_size", default=128, type=int, help="Total batch size for eval.")
    ...
    parser.add_argument("--report_dir", default='baselines/las/training_reports/', type=str,
    ...
    if args.do_eval:
        ...
        stats_dict = train_or_eval_epoch(args, device, dev_dataloader, stats_dict, multi_gpu = multi_gpu, 
                               ...
                               split_name = 'dev')   
        if 'QA' not in data_name or args.labels_to_use != 'label':
        ...
        if 'QA' not in data_name or args.labels_to_use != 'label': # CQA does not have test labels. so compute test on other datasets and CQA simulation
            if args.do_task:
