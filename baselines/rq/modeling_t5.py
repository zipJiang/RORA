...
"""
    A modified version of the Python file of the same name from Huggingface Transformers v2.9.1 (https://github.com/huggingface/transformers/blob/v2.9.1/src/transformers/modeling_t5.py)
    Modifications made to the forward functions of the T5ForConditionalGeneration and the T5Stack classes to accept noising and variance, as well as to compute gradients on the logits.
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value_state is not None:
            ...
            real_qlen = (
                qlen + past_key_value_state[0].shape[2]
    ...
    def forward(
        ...
        if past_key_value_state is not None:
            ...
            expected_num_past_key_value_states = (
                2 if encoder_hidden_states is None else 4
            )
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        # if noisy == True, add 0-mean Gaussian noise to embeds
        if noisy:
            inputs_embeds = inputs_embeds + (
                torch.randn(inputs_embeds.size()).to(inputs_embeds.device) * variance
            )
        ...
        if (
            ...
            encoder_attention_mask = torch.ones(batch_size, encoder_seq_length).to(
                inputs_embeds.device
            )
            ...
            if i == 0:
                ...
                if self.is_decoder and encoder_hidden_states is not None:
                    encoder_decoder_position_bias = layer_outputs[
                        4 if self.output_attentions else 3
                    ]
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            raise Exception(
                "This condition should not be triggered during noisy inference."
            )
            encoder_outputs = self.encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
        ...
        decoder_outputs = (lm_logits,) + decoder_outputs[
            1:
        ...
        if labels is not None:
            if not self.gradients:
                ...
                loss = 0
                indices = (labels != -100).nonzero()
                for inx in indices:
                    score_pred_class = lm_logits[inx[0], inx[1]]
                    pred_label = labels[inx[0], inx[1]]
                    loss_adder = score_pred_class[pred_label]
                    loss += loss_adder
                    # the below are not true when we noise the input embeddings for SmoothGrad
                    # assert torch.eq(torch.max(score_pred_class, dim=-1).values, loss_adder)
                    # assert torch.eq(torch.max(score_pred_class, dim=-1).indices, pred_label)
                # take mean
                loss = loss / float(len(indices))
    ...
    """ Override generation function in modeling_utils.py for class PreTrainedModel
    (Inheritance: PreTrainedModel --> T5PreTrainedModel --> T5ForConditionalGeneration)
    These two below generation functions are taken and modified from Transformers v2.9.1 modeling_utils file (https://github.com/huggingface/transformers/blob/v2.9.1/src/transformers/modeling_utils.py)
    """
    ...
    def generate(
        ...
        **model_specific_kwargs,
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs: tuple = encoder(**tmp)
        ...
        if self.config.is_encoder_decoder:
            ...
            raise Exception("not an encoder-decoder; must move position of noise.")
            encoder_outputs = None
            cur_len = input_ids.shape[-1]

        if num_beams > 1:
            raise Exception("beam search not implemented")
        else:
    ...
    def _generate_no_beam_search(
        ...
        while cur_len < max_length:
            ...
            if variance is not None:
                model_inputs["noisy"] = True
            else:
                model_inputs["noisy"] = False
            model_inputs["variance"] = variance
