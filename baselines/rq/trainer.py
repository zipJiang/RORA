...
from custom_saliency_interpreters.simple_gradient import SimpleGradient
from custom_saliency_interpreters.smooth_gradient import SmoothGradient
from custom_saliency_interpreters.integrated_gradient import IntegratedGradient
...
class Trainer:
    ...
    def compute_gradients(self, save_path, dataset, split, task_name, model_args):
        # get file for saving
        fname = os.path.join(
            save_path,
            "%s_%s_%s_%s_attributions.txt"
            % (
                task_name,
                split,
                model_args.gradient_method,
                model_args.combination_method,
            ),
        )
        if os.path.isfile(fname):
            fname = fname.split(".txt")[0] + "_1.txt"
        if os.path.isfile(fname):
            raise Exception("both gradient files exist already")

        # generate gradients
        model = self.model
        # set gradients attribute in model forward pass
        model.gradients = True
        model.eval()

        assert dataset[0]["question_encoding"] == dataset[0]["input_ids"]

        # set batch_size to 1, since conditional decoding is individual (NOTE: slow)
        # this will also convert everything to torch.Tensor of shape [1, length] without padding
        dataloader = DataLoader(
            dataset,
        ...
        predictor = Predictor(
            model=model,
            dataset_reader=dataloader,
            tokenizer=self.tokenizer,
            device=self.args.device,
        )
        if model_args.gradient_method in {"raw", "times_input"}:
            gradients = SimpleGradient(predictor)
            gradients = gradients.saliency_interpret_from_json(
                dataloader,
                multiply=(model_args.gradient_method == "times_input"),
                l1=(model_args.combination_method == "l1"),
            )

        elif model_args.gradient_method in {"smoothgrad", "smoothgrad_squared"}:
            gradients = SmoothGradient(
                predictor, model_args.smoothgrad_stdev, model_args.nsamples
            )
            gradients = gradients.saliency_interpret_from_json(
                dataloader,
                squared=(model_args.gradient_method == "smoothgrad_squared"),
                l1=(model_args.combination_method == "l1"),
            )

        elif model_args.gradient_method == "integrated":
            gradients = IntegratedGradient(predictor)
            gradients = gradients.saliency_interpret_from_json(
                dataloader,
                model_args.nsamples,
                l1=(model_args.combination_method == "l1"),
            ...
            raise Exception("viable gradient method not specified")

        # write-out gradient attributions to a JSON file
        logger.info("Writing gradient attributions to %s" % fname)
        with open(fname, "w") as outfile:
            json.dump(gradients, outfile)
